...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
   ...
   <% using (Html.BeginForm()){%>
        ...
        <%= Html.GridJQuery("Descuento")%>  
        <%= Html.EndSectionBody()%> 
        <%= Html.BeginBarButtons()%>
        <%= Html.ActionLinkCreate()%>
        <%= Html.ActionLinkPrintGrid("Descuento")%>
        <%= Html.ActionLinkHome()%>      
        <%= Html.EndBarButtons()%>
        <%= Html.EndSection()%>
    <%}%>
...
<asp:Content ID="Content3" ContentPlaceHolderID="HeadContent" runat="server">
	<script type='text/javascript'>
	    ...
	    $(function () {
	        var jqTableGrid = mvcLocal.jqGrid.init("Descuento");
	        //
	        // Specify the column names
	        jqTableGrid.mvcUI.config.options.colNames = ["ID", "Codigo", "Descripcion",	"PorcentajeDescuento",
                                                          "Estatus", "Acciones"];
	        //
	        ...
	        jqTableGrid.mvcUI.config.options.colModel =
				[
					...
					{ name: "PorcentajeDescuento", index: "PorcentajeDescuento", width: 70, align: "left" },
                    { name: "Estatus", index: "Estatus", width: 70, align: "left" },
