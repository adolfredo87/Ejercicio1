<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<AlquileresMVC.Models.Tipo>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
   ...
   <% using (Html.BeginForm()){%>
        ...
        <%= Html.GridJQuery("Tipo")%>  
        <%= Html.EndSectionBody()%> 
        <%= Html.BeginBarButtons()%>
        <%= Html.ActionLinkCreate()%>
        <%= Html.ActionLinkPrintGrid("Tipo")%>
        <%= Html.ActionLinkHome()%>      
        <%= Html.EndBarButtons()%>
        <%= Html.EndSection()%>
    <%}%>
...
<asp:Content ID="Content3" ContentPlaceHolderID="HeadContent" runat="server">
	<script type='text/javascript'>
	    ...
	    $(function () {
	        var jqTableGrid = mvcLocal.jqGrid.init("Tipo");
	        //
